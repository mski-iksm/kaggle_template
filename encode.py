...
from codes.utils.misc import get_current_commit_hash
...
def run(command):
    os.system('echo "from setuptools import setup; setup(name=\\'codes\\', packages=[\\'codes\\'],)" > setup.py')
...
def build_script():
    to_encode = list(Path('codes').glob('**/*.py'))
